...
export function createLogger({name='app', level='info', sinks=[]} = {}){
  ...
  return {
    ...
    child(suffix, extra = {}){
      const childLogger = createLogger({name: `${name}:${suffix}`, level});
      childLogger.addSink((record)=>{
        emit(record.level, record.event, {...extra, ...record.data});
      });
      return childLogger;
    }
  };
